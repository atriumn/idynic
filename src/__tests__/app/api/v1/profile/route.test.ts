...
const mockClaims = [
  { id: 'claim-1', user_id: 'user-123', type: 'skill', label: 'TypeScript', description: 'Expert level', confidence: 0.95 },
  { id: 'claim-2', user_id: 'user-123', type: 'skill', label: 'React', description: 'Advanced', confidence: 0.90 },
  { id: 'claim-3', user_id: 'user-123', type: 'education', label: 'BS Computer Science', description: 'Stanford University', confidence: 0.99 },
  { id: 'claim-4', user_id: 'user-123', type: 'certification', label: 'AWS Solutions Architect', description: 'Professional', confidence: 0.98 }
]
...
describe('Profile API Route', () => {
  ...
  describe('GET /api/v1/profile', () => {
    it('returns profile with all related data', async () => {
      mockSupabaseFrom.mockImplementation((table: string) => {
        ...
        if (table === 'work_history') {
          return {
            select: vi.fn().mockReturnValue({
              eq: vi.fn().mockReturnValue({
                order: vi.fn().mockResolvedValue({ data: mockWorkHistory, error: null })
              })
            })
          }
        }
      ...
      const body = await parseJsonResponse<{
        ...
        data: {
          ...
          skills: Array<{ id: string; label: string; description: string; confidence: number }>
          education: Array<{ id: string; label: string; description: string; confidence: number }>
          certifications: Array<{ id: string; label: string; description: string; confidence: number }>
        }
      }>(response)
    ...
    it('handles null claims data', async () => {
      ...
      const body = await parseJsonResponse<{
        success: boolean
        data: { skills: unknown[]; education: unknown[]; certifications: unknown[] }
      }>(response)
  ...
  describe('PATCH /api/v1/profile', () => {
    ...
    it('handles invalid JSON body', async () => {
      ...
      const request = new NextRequest(new URL('/api/v1/profile', 'http://localhost:3000'), {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer idn_test123' },
        body: 'not valid json'
      })
