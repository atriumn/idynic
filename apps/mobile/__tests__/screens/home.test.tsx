...
jest.mock('../../hooks/use-identity-claims', () => ({
  ...
  CLAIM_TYPE_COLORS: {
    skill: { bgHex: '#064e3b', borderHex: '#047857', textHex: '#6ee7b7', icon: '#10b981' },
    achievement: { bgHex: '#713f12', borderHex: '#a16207', textHex: '#fcd34d', icon: '#f59e0b' },
    attribute: { bgHex: '#1e3a8a', borderHex: '#1d4ed8', textHex: '#93c5fd', icon: '#3b82f6' },
    education: { bgHex: '#581c87', borderHex: '#7c3aed', textHex: '#c4b5fd', icon: '#8b5cf6' },
    certification: { bgHex: '#881337', borderHex: '#be123c', textHex: '#fda4af', icon: '#f43f5e' },
  },
  CLAIM_TYPE_LABELS: {
    skill: 'Skills',
...
describe('IdentityScreen (Home)', () => {
  ...
  it('shows total claim count', () => {
    (useIdentityClaims as jest.Mock).mockReturnValue({
      data: {
        skill: [
          { id: '1', type: 'skill', label: 'React', confidence: 0.9, evidence: [] },
          { id: '2', type: 'skill', label: 'Node', confidence: 0.8, evidence: [] },
        ],
        achievement: [
          { id: '3', type: 'achievement', label: 'Led team', confidence: 0.85, evidence: [] },
        ],
        attribute: [],
        education: [],
        certification: [],
  ...
  it('filters claims by search query', () => {
    (useIdentityClaims as jest.Mock).mockReturnValue({
      data: {
        skill: [
          { id: '1', type: 'skill', label: 'React', description: 'Frontend', confidence: 0.9, evidence: [] },
          { id: '2', type: 'skill', label: 'Python', description: 'Backend', confidence: 0.8, evidence: [] },
        ],
        achievement: [],
        attribute: [],
        education: [],
  ...
  it('shows no results message when search has no matches', () => {
    ...
    fireEvent.changeText(screen.getByPlaceholderText('Search claims...'), 'xyz123');
  ...
  it('expands claim card to show details', () => {
        ...
        skill: [
          {
            ...
            type: 'skill',
            label: 'React',
            description: 'Expert in React development with hooks and modern patterns',
            confidence: 0.9,
            evidence: [
              { id: 'ev-1', text: 'Built complex SPAs', source_type: 'resume', evidence_type: 'direct' },
            ],
          },
        ],
        achievement: [],
    ...
    expect(screen.getByText('Supporting Evidence (1)')).toBeTruthy();
    expect(screen.getByText('Built complex SPAs')).toBeTruthy();
  });
  ...
  it('toggles claim type filter', () => {
    (useIdentityClaims as jest.Mock).mockReturnValue({
      data: {
        ...
        achievement: [
          { id: '2', type: 'achievement', label: 'Led Project', confidence: 0.85, evidence: [] },
        ],
        attribute: [],
        education: [],
        certification: [],
  ...
  it('shows Show All button when filters are active', () => {
    (useIdentityClaims as jest.Mock).mockReturnValue({
      data: {
        skill: [{ id: '1', type: 'skill', label: 'React', confidence: 0.9, evidence: [] }],
        achievement: [{ id: '2', type: 'achievement', label: 'Led', confidence: 0.85, evidence: [] }],
        attribute: [],
        education: [],
        certification: [],
      },
  ...
  it('collapses section when header is pressed', () => {
    (useIdentityClaims as jest.Mock).mockReturnValue({
      data: {
        skill: [
          { id: '1', type: 'skill', label: 'React', confidence: 0.9, evidence: [] },
          { id: '2', type: 'skill', label: 'TypeScript', confidence: 0.85, evidence: [] },
        ],
        achievement: [],
        attribute: [],
        education: [],
