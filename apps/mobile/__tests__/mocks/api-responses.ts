...
import type { GroupedClaims, IdentityClaim, Evidence } from '../../hooks/use-identity-claims';
...
export const mockProfile: Profile = {
  ...
  skills: [
    ...
    { id: 'skill-2', label: 'React', description: null, confidence: 0.90 },
  ],
  education: [
    { id: 'edu-1', text: 'BS Computer Science, Stanford University', context: {} },
  ],
};
...
export function createSupabaseResponse<T>(data: T, error: null): { data: T; error: null };
export function createSupabaseResponse<T>(data: null, error: { message: string; code?: string }): { data: null; error: { message: string; code?: string } };
export function createSupabaseResponse<T>(data: T | null, error: { message: string; code?: string } | null) {
  return { data, error };
}
