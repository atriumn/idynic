...
describe("normalizeJobUrl", () => {
  describe("LinkedIn URLs", () => {
    ...
    it("strips tracking params from LinkedIn URL", () => {
      const url =
        "https://www.linkedin.com/jobs/view/3847291034?refId=abc&trackingId=xyz&trk=jobs_list";
      expect(normalizeJobUrl(url)).toBe("linkedin:3847291034");
    });
    ...
    it("handles LinkedIn job collection URLs", () => {
      const url =
        "https://www.linkedin.com/jobs/collections/recommended/?currentJobId=3847291034";
  ...
  describe("Lever URLs", () => {
    it("extracts company and job ID from Lever URL", () => {
      ...
      expect(normalizeJobUrl(url)).toBe(
        "lever:acme:a1b2c3d4-e5f6-7890-abcd-ef1234567890",
      );
    });

    it("handles Lever URLs with query params", () => {
      const url =
        "https://jobs.lever.co/startup/f1a2b3c4-d5e6-7890-abcd-1234567890ab?lever-origin=applied";
      expect(normalizeJobUrl(url)).toBe(
        "lever:startup:f1a2b3c4-d5e6-7890-abcd-1234567890ab",
      );
    });

    it("handles company names with hyphens", () => {
      const url =
        "https://jobs.lever.co/acme-corp/12345678-1234-1234-1234-123456789012";
      expect(normalizeJobUrl(url)).toBe(
        "lever:acme-corp:12345678-1234-1234-1234-123456789012",
      );
    });

    it("handles Lever main domain", () => {
      const url =
        "https://lever.co/company/98765432-abcd-ef01-2345-678901234567";
      expect(normalizeJobUrl(url)).toBe(
        "lever:company:98765432-abcd-ef01-2345-678901234567",
      );
    });
  ...
  describe("Other URLs", () => {
    it("strips query params and keeps hostname + path", () => {
      const url =
        "https://careers.example.com/jobs/senior-engineer?ref=twitter";
      expect(normalizeJobUrl(url)).toBe(
        "careers.example.com/jobs/senior-engineer",
      );
    });
    ...
    it("handles URLs with encoded characters", () => {
      ...
      expect(normalizeJobUrl(url)).toBe(
        "careers.example.com/jobs/senior%20engineer",
      );
    });

    it("handles Workday URLs", () => {
      const url =
        "https://acme.myworkdayjobs.com/en-US/External/job/USA/Engineer_JR-12345";
      expect(normalizeJobUrl(url)).toBe(
        "acme.myworkdayjobs.com/en-US/External/job/USA/Engineer_JR-12345",
      );
    });
    ...
    it("handles Glassdoor URLs", () => {
      const url =
        "https://www.glassdoor.com/job-listing/software-engineer-acme-JV_123456789.htm?jl=123456789";
      expect(normalizeJobUrl(url)).toBe(
        "www.glassdoor.com/job-listing/software-engineer-acme-JV_123456789.htm",
      );
    });
  });
