...
describe("Share Link API Route", () => {
  ...
  describe("POST /api/v1/opportunities/[id]/share", () => {
    it("creates new share link successfully", async () => {
      ...
      const { POST } =
        await import("@/app/api/v1/opportunities/[id]/share/route");
            ...
            select: vi.fn().mockReturnValue({
              eq: vi.fn().mockReturnValue({
                is: vi.fn().mockReturnValue({
                  single: vi
                    .fn()
                    .mockResolvedValue({ data: mockExistingLink, error: null }),
                }),
              }),
            }),
          };
    ...
    it("handles empty request body", async () => {
      ...
      const request = new NextRequest(
        new URL("/api/v1/opportunities/opp-123/share", "http://localhost:3000"),
        {
          method: "POST",
          headers: { Authorization: "Bearer idn_test123" },
        },
