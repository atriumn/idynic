...
vi.mock('@/lib/ai/match-opportunity-api', () => ({
  computeOpportunityMatchesWithClient: (...args: unknown[]) => mockComputeOpportunityMatches(...args)
}))
...
const mockMatchResult = {
  ...
  gaps: [
    ...
    { text: 'AWS certification', type: 'certification', category: 'niceToHave' }
  ]
}
...
describe('Match API Route', () => {
  ...
  describe('GET /api/v1/opportunities/[id]/match', () => {
    it('returns match scores and analysis', async () => {
      ...
      const { GET } = await import('@/app/api/v1/opportunities/[id]/match/route')
      ...
      const body = await parseJsonResponse<{
        ...
        data: {
          ...
          strengths: Array<{ requirement: string; match: { claim: string; similarity: number } | null }>
    ...
    it('returns 404 when opportunity not found', async () => {
      ...
      const { GET } = await import('@/app/api/v1/opportunities/[id]/match/route')
      const request = createMockRequest('/api/v1/opportunities/nonexistent/match', {
        headers: { 'Authorization': 'Bearer idn_test123' }
      })
    ...
    it('limits strengths to top 5', async () => {
      const manyStrengths = Array.from({ length: 10 }, (_, i) => ({
        ...
        bestMatch: { label: `Match ${i}`, type: 'skill', similarity: 0.9 - i * 0.05 }
      }))
