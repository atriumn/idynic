...
describe('useSkillClusters', () => {
  ...
  describe('stale time configuration', () => {
    it('uses configured stale time', async () => {
      ...
      const query = queryClient.getQueryCache().find({ queryKey: ['skill-clusters'] })
  ...
  describe('data structure handling', () => {
    ...
    it('handles partial embedding data', async () => {
      ...
      expect(result.current.data?.message).toBe('Some claims are missing embeddings')
    })
  })
  ...
  describe('node types', () => {
    it('handles various node types', async () => {
      const mixedTypesData = {
        nodes: [
          { id: '1', label: 'TypeScript', type: 'skill', confidence: 0.9, x: 0, y: 0 },
          { id: '2', label: 'AWS', type: 'expertise', confidence: 0.8, x: 1, y: 1 },
          { id: '3', label: 'Leadership', type: 'soft_skill', confidence: 0.85, x: 2, y: 2 },
        ],
        hasEmbeddings: true,
      }
  ...
  describe('confidence values', () => {
    it('handles various confidence levels', async () => {
      const confidenceData = {
        nodes: [
          { id: '1', label: 'High', type: 'skill', confidence: 0.95, x: 0, y: 0 },
          { id: '2', label: 'Medium', type: 'skill', confidence: 0.65, x: 1, y: 1 },
          { id: '3', label: 'Low', type: 'skill', confidence: 0.35, x: 2, y: 2 },
        ],
        hasEmbeddings: true,
      }
