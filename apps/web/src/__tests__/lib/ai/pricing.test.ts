...
describe("AI_PRICING constants", () => {
  it("should have OpenAI models defined", () => {
    ...
    expect(AI_PRICING.openai["gpt-4o-mini"]).toEqual({
      input: 0.15,
      output: 0.6,
    });
  ...
  it("should have embedding models defined", () => {
    expect(AI_PRICING.openai["text-embedding-3-small"]).toEqual({
      input: 0.02,
      output: 0,
    });
    expect(AI_PRICING.openai["text-embedding-3-large"]).toEqual({
      input: 0.13,
      output: 0,
    });
  });
  ...
  it("should have Anthropic models defined", () => {
    ...
    expect(AI_PRICING.anthropic["claude-sonnet-4-5-20250514"]).toEqual({
      input: 3.0,
      output: 15.0,
    });
  });
  ...
  it("should have Google models defined", () => {
    ...
    expect(AI_PRICING.google["gemini-3-flash-preview"]).toEqual({
      input: 0.5,
      output: 3.0,
    });
  });
...
describe("calculateCostCents", () => {
  ...
  it("should return 0 and warn for unknown provider", () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(
      "Unknown pricing for unknown-provider/some-model",
    );
  });
  ...
  it("should return 0 and warn for unknown model", () => {
    ...
    expect(cost).toBe(0);
    expect(console.warn).toHaveBeenCalledWith(
      "Unknown pricing for openai/unknown-model",
    );
  });
