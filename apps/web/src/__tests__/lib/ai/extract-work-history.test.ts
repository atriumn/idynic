  ...
  describe("extractWorkHistory", () => {
    it("successfully extracts work history", async () => {
      ...
      const { extractWorkHistory } =
        await import("@/lib/ai/extract-work-history");
    ...
    it("throws error when no response content", async () => {
      ...
      await expect(extractWorkHistory("Resume text")).rejects.toThrow(
    ...
    it("handles all entry types correctly", async () => {
      ...
      expect(result.map((j) => j.entry_type)).toEqual([
        "work",
        "venture",
        "additional",
      ]);
