  ...
  gaps: [
    {
      ...
      mitigation:
        "Strong Docker experience demonstrates container fundamentals",
  ...
  inferences: [
    {
      ...
      reasoning:
        "Multiple executive presentations imply stakeholder management",
    },
  ],
};
  ...
  describe("generateTalkingPoints", () => {
    it("returns empty talking points when opportunity has no requirements", async () => {
      ...
      const { generateTalkingPoints } =
        await import("@/lib/ai/generate-talking-points");
    ...
    it("throws error when opportunity fails to load", async () => {
      ...
      await expect(
        generateTalkingPoints("opp-123", "user-456"),
