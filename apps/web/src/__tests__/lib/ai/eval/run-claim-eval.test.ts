...
vi.mock("@/lib/ai/eval/claim-grounding", () => ({
  runClaimGroundingEval: vi
    .fn()
    .mockResolvedValue({ issues: [], costCents: 5 }),
}));
...
describe("run-claim-eval", () => {
  ...
  describe("evidence query structure", () => {
    it("should query claim_evidence with strength and evidence.text", async () => {
      ...
      const claimsWithEvidence = [
        ...
        },
        {
          ...
          claim_evidence: [
            {
              strength: "medium",
              evidence: { text: "Used Python for scripts" },
            },
          ],
        },
      ];
      ...
      mockSupabase._mockIn.mockResolvedValueOnce({
        data: claimsWithEvidence,
        error: null,
      });

      await runClaimEval(
        mockSupabase as unknown as Parameters<typeof runClaimEval>[0],
        "user-123",
        "doc-123",
      );
  ...
  describe("error handling", () => {
    it("should return empty result when claims fetch fails", async () => {
      ...
      const result = await runClaimEval(
        mockSupabase as unknown as Parameters<typeof runClaimEval>[0],
        "user-123",
        "doc-123",
      );
    ...
    it("should continue when evidence fetch fails but log error", async () => {
      ...
      mockSupabase._mockIn.mockResolvedValueOnce({
        data: null,
        error: { message: "Evidence fetch failed" },
      });
  ...
  describe("sampling", () => {
    it("should respect maxClaimsForAiEval option", async () => {
      ...
      const claimsWithEvidence = Array.from({ length: 5 }, (_, i) => ({
        ...
        description: null,
        claim_evidence: [
          { strength: "strong", evidence: { text: `Evidence ${i}` } },
        ],
      }));
