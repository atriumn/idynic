...
import { searchTavily, type TavilySearchParams, type TavilyResponse } from '@/lib/integrations/tavily';
...
describe('searchTavily', () => {
  ...
  it('should throw error when TAVILY_API_KEY is not configured', async () => {
    ...
    await expect(searchTavily({ query: 'test' })).rejects.toThrow('TAVILY_API_KEY not configured');
  });
  ...
  it('should throw error on API failure', async () => {
    ...
    await expect(searchTavily({ query: 'test' })).rejects.toThrow('Tavily API error: 401 - Unauthorized');
  });
  ...
  it('should throw error on 500 server error', async () => {
    ...
    await expect(searchTavily({ query: 'test' })).rejects.toThrow('Tavily API error: 500 - Internal Server Error');
  });
});
