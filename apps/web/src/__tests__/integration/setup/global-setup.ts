...
export default async function globalSetup() {
  ...
  if (!isRunning) {
    ...
    try {
      ...
      throw new Error(
        'Failed to start Supabase. Run "supabase start" manually and try again.',
      );
    }
  } else {
  ...
  return async () => {
    ...
    console.log(
      '[Integration Tests] Note: Supabase is still running. Run "supabase stop" to stop it.\n',
    );
  };
}
...
async function applyTestFixtures(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[Integration Tests] Could not apply fixtures via CLI, continuing...",
      );
    }
  }
}
...
async function reloadPostgrestSchema(): Promise<void> {
  ...
  try {
    ...
    if (containerId) {
      console.log(
        `[Integration Tests] Restarting PostgREST container: ${containerId}`,
      );
    ...
    console.warn(
      "[Integration Tests] Failed to restart PostgREST container:",
      error,
    );
  }
  ...
  if (!restartSucceeded) {
    ...
    try {
      execSync(
        `psql "${getPostgresConnectionString()}" -c "NOTIFY pgrst, 'reload schema'"`,
        {
          cwd: getRepoRoot(),
          stdio: "pipe",
          timeout: 10000,
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (!error) {
      console.log(
        `[Integration Tests] Schema loaded successfully (attempt ${attempt}/${maxAttempts})`,
      );
      return;
    }
    ...
    if (error.code !== "PGRST205") {
      ...
      console.log(
        `[Integration Tests] Schema loaded with expected error: ${error.code} (attempt ${attempt}/${maxAttempts})`,
      );
      return;
    }

    console.log(
      `[Integration Tests] Schema not yet loaded (attempt ${attempt}/${maxAttempts}), waiting...`,
    );

    // On attempt 15, try restarting the PostgREST container again
    if (attempt === 15) {
      console.log(
        "[Integration Tests] Retrying PostgREST container restart...",
      );
      try {
        const containerId = execSync(
          'docker ps --filter "name=supabase_rest" --format "{{.ID}}"',
        ...
        if (containerId) {
          ...
          console.log(
            "[Integration Tests] PostgREST container restarted again",
          );
          await new Promise((resolve) => setTimeout(resolve, 5000));
        }
      } catch {
        console.warn(
          "[Integration Tests] Could not restart PostgREST container",
        );
      }
    }
  ...
  throw new Error(
    `PostgREST schema cache did not reload in time after ${maxAttempts} attempts`,
  );
}
