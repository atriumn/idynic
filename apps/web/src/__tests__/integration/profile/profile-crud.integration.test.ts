...
describe('Profile CRUD Operations', () => {
  ...
  describe('Update Profile', () => {
    ...
    it('should allow setting fields to null', async () => {
      // First set a value
      await client.from('profiles').update({ phone: '+1-555-555-5555' }).eq('id', user.id)
  ...
  describe('Work History Integration', () => {
    ...
    it('should delete work history entry', async () => {
      ...
      // Delete it
      const { error } = await client.from('work_history').delete().eq('id', created!.id)
...
describe('Profile API Key Management', () => {
  ...
  beforeAll(async () => {
    const result = await createTestUserWithClient(generateTestEmail('api-keys'))
    user = result.user
    client = result.client
  })
