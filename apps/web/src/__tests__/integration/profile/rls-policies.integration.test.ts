...
describe('RLS Policies: Profiles', () => {
  ...
  beforeAll(async () => {
    ...
    const result1 = await createTestUserWithClient(generateTestEmail('rls-user1'))
    user1 = result1.user
    user1Client = result1.client

    // User2 is created to test cross-user RLS policies
    const result2 = await createTestUserWithClient(generateTestEmail('rls-user2'))
    user2 = result2.user
...
describe('RLS Policies: Documents', () => {
  ...
  beforeAll(async () => {
    const result1 = await createTestUserWithClient(generateTestEmail('rls-docs-user1'))
    user1 = result1.user
    user1Client = result1.client

    const result2 = await createTestUserWithClient(generateTestEmail('rls-docs-user2'))
    user2 = result2.user
    user2Client = result2.client
  ...
  it('should allow user to DELETE their own documents', async () => {
    ...
    const { error } = await user1Client.from('documents').delete().eq('id', newDoc!.id)
...
describe('RLS Policies: Opportunities', () => {
  ...
  beforeAll(async () => {
    const result1 = await createTestUserWithClient(generateTestEmail('rls-opp-user1'))
    user1 = result1.user
    user1Client = result1.client

    const result2 = await createTestUserWithClient(generateTestEmail('rls-opp-user2'))
    user2 = result2.user
    user2Client = result2.client
  ...
  it('should allow user to DELETE their own opportunities', async () => {
    ...
    const { error } = await user1Client.from('opportunities').delete().eq('id', newOpp!.id)
