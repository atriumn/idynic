...
vi.mock('@/components/editable-text', () => ({
  EditableText: ({ value, fieldPath }: { value: string; fieldPath: string }) => (
...
vi.mock('@/components/regenerate-warning-dialog', () => ({
  RegenerateWarningDialog: ({ open, onConfirm }: { open: boolean; onConfirm: () => void }) =>
    open ? (
...
describe('TailoredProfile', () => {
  ...
  describe('no profile state', () => {
    it('shows generate button when no profile exists', async () => {
      ...
      expect(screen.getByRole('button', { name: /generate tailored profile/i })).toBeInTheDocument()
    })
    ...
    it('generates profile when button is clicked', async () => {
      ...
      await user.click(screen.getByRole('button', { name: /generate tailored profile/i }))
  ...
  describe('profile loaded state', () => {
    ...
    describe('narrative tab', () => {
      it('shows narrative content', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByText(/cover letter narrative/i)).toBeInTheDocument()
        })
      })
    ...
    describe('resume tab', () => {
      ...
      it('shows PDF preview toggle button', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByRole('button', { name: /pdf preview/i })).toBeInTheDocument()
        })
      })
  ...
  describe('error state', () => {
    it('shows error message on fetch failure', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/failed to generate profile/i)).toBeInTheDocument()
      })
    })
  ...
  describe('evaluation banner', () => {
    it('shows success banner when evaluation passes', async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            profile: mockProfile,
            evaluation: { passed: true, groundingPassed: true, hallucinations: null },
          }),
      })
    ...
    it('shows warning banner when hallucinations detected', async () => {
        ...
        json: () =>
          Promise.resolve({
            ...
            evaluation: {
              ...
              hallucinations: [
                { field: 'summary', claim: 'Python expert', reason: 'Not found in documents' },
              ],
            },
          }),
      })
      ...
      await waitFor(() => {
        expect(screen.getByText(/potential accuracy issues detected/i)).toBeInTheDocument()
    ...
    it('can dismiss warning banner', async () => {
        ...
        json: () =>
          Promise.resolve({
            ...
            evaluation: {
              ...
              groundingPassed: false,
              hallucinations: [
                { field: 'summary', claim: 'Python expert', reason: 'Not found' },
              ],
            },
          }),
      })
      ...
      await waitFor(() => {
        expect(screen.getByText(/potential accuracy issues/i)).toBeInTheDocument()
      })
  ...
  describe('requirement matches', () => {
    it('shows required qualifications section when provided', async () => {
      const requirementMatches = [
        {
          requirement: { text: '5+ years experience', type: 'experience', category: 'mustHave' as const },
          bestMatch: { id: 'claim-1', type: 'skill', label: 'Experience', description: null, confidence: 0.9, similarity: 0.85 },
        },
      ]
      ...
      render(<TailoredProfile opportunityId="opp-123" requirementMatches={requirementMatches} />)
    ...
    it('shows nice to have section when provided', async () => {
      const requirementMatches = [
        {
          requirement: { text: 'GraphQL experience', type: 'skill', category: 'niceToHave' as const },
          bestMatch: null,
        },
      ]
