...
vi.mock('@/components/edit-claim-modal', () => ({
  EditClaimModal: ({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) =>
    open ? <div data-testid="edit-modal"><button onClick={() => onOpenChange(false)}>Close</button></div> : null,
}))
...
describe('IdentityClaimsList', () => {
  ...
  it('filters claims by search query', async () => {
    ...
    expect(screen.queryByText('Led team of 5 engineers')).not.toBeInTheDocument()
  })
  ...
  it('shows issues badge when claims have issues', () => {
    ...
    // Should show "1 issue" button since one claim has an issue
  ...
  it('toggles issues filter when issues button is clicked', async () => {
    ...
    await user.click(screen.getByText(/1 issue/i))
  ...
  it('shows claim description in collapsed state', () => {
    ...
    expect(screen.getByText('Expert in React and its ecosystem')).toBeInTheDocument()
  })
  ...
  describe('expanded claim with issues', () => {
    ...
    it('dismisses issues when dismiss is clicked', async () => {
      ...
      const user = userEvent.setup()
      render(<IdentityClaimsList claims={mockClaims} onClaimUpdated={onClaimUpdated} />)
      ...
      await waitFor(() => {
        expect(mockFetch).toHaveBeenCalledWith('/api/v1/claims/claim-3/dismiss', {
          method: 'POST',
