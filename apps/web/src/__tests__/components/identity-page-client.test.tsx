...
vi.mock('@/lib/hooks/use-identity-graph', () => ({
  useIdentityGraph: () => ({
    data: {
      nodes: [
        { id: 'claim-1', label: 'React', type: 'skill', confidence: 0.9, claim_evidence: [], issues: [] },
        { id: 'claim-2', label: 'TypeScript', type: 'skill', confidence: 0.85, claim_evidence: [], issues: [] },
      ],
      documents: [],
    },
    isLoading: false,
...
vi.mock('@/components/identity-constellation', () => ({
  IdentityConstellation: () => <div data-testid="identity-constellation">Constellation</div>,
}))

vi.mock('@/components/evidence-constellation', () => ({
  EvidenceConstellation: () => <div data-testid="evidence-constellation">Evidence</div>,
}))

vi.mock('@/components/confidence-sunburst', () => ({
  ConfidenceSunburst: () => <div data-testid="confidence-sunburst">Sunburst</div>,
}))
...
vi.mock('@/components/claim-detail-panel', () => ({
  ClaimDetailPanel: () => <div data-testid="claim-detail-panel">Detail Panel</div>,
}))

vi.mock('@/components/upload-resume-modal', () => ({
  UploadResumeModal: () => <button data-testid="upload-resume-modal">Upload Resume</button>,
}))
...
vi.mock('@/components/identity/identity-reflection', () => ({
  IdentityReflection: ({ data }: { data: { headline?: string } | null }) => (
    <div data-testid="identity-reflection">{data?.headline || 'No reflection'}</div>
  ),
}))
...
describe('IdentityPageClient', () => {
  ...
  describe('view switching', () => {
    it('can switch to treemap view', async () => {
      ...
      const treemapButton = buttons.find((btn) => btn.getAttribute('title')?.includes('Treemap'))
      if (treemapButton) {
        await user.click(treemapButton)

        await waitFor(() => {
          expect(screen.getByTestId('identity-constellation')).toBeInTheDocument()
        })
      }
    })
    ...
    it('can switch to radial view', async () => {
      ...
      const buttons = screen.getAllByRole('button')
      const radialButton = buttons.find((btn) => btn.getAttribute('title')?.includes('Radial'))
      if (radialButton) {
        await user.click(radialButton)

        await waitFor(() => {
          expect(screen.getByTestId('evidence-constellation')).toBeInTheDocument()
        })
      }
    })
    ...
    it('can switch to sunburst view', async () => {
      ...
      const buttons = screen.getAllByRole('button')
      const sunburstButton = buttons.find((btn) => btn.getAttribute('title')?.includes('Sunburst'))
      if (sunburstButton) {
    ...
    it('can switch to clusters view', async () => {
      ...
      const buttons = screen.getAllByRole('button')
      const clustersButton = buttons.find((btn) => btn.getAttribute('title')?.includes('Clusters'))
      if (clustersButton) {
