...
export function OpportunityList({
  ...
  const allStatuses = Array.from(
    new Set(initialOpportunities.map((o) => o.status || "tracking")),
  );
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-muted/30 p-4 rounded-2xl border-2 border-muted/50">
        <div className="relative w-full sm:w-96">
          ...
          <Input
            placeholder="Search targets, roles, or companies..."
            className="pl-10 h-11 bg-background border-none shadow-inner focus-visible:ring-1 font-medium"
            value={searchQuery}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="h-11 px-4 font-bold border-2 bg-background"
              >
                ...
                {statusFilter.length > 0 && (
                  <Badge
                    variant="secondary"
                    className="ml-2 h-5 px-1.5 rounded-sm text-xs font-black"
                  >
                    {statusFilter.length}
                  </Badge>
                )}
            ...
            <DropdownMenuContent align="end" className="w-[200px] p-2">
              <DropdownMenuItem
                ...
                className="justify-center text-[10px] font-black uppercase tracking-widest text-muted-foreground"
              >
                Reset All
              </DropdownMenuItem>
              <DropdownMenuSeparator />
      ...
      {filteredOpportunities.length === 0 ? (
        <div className="text-center py-24 border-2 border-dashed rounded-3xl bg-muted/5">
          ...
          <h3 className="text-xl font-black tracking-tight">
            Zero targets match your search
          </h3>
          <p className="text-muted-foreground max-w-xs mx-auto mt-1 text-base font-medium">
            Try expanding your search criteria or adjusting the status filters.
          </p>
        </div>
      ) : viewMode === "grid" ? (
          ...
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow className="hover:bg-transparent border-b-2">
                ...
                <TableHead className="text-right font-black text-[10px] uppercase tracking-widest h-12">
                  Action
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredOpportunities.map((opp) => (
                <TableRow
                  key={opp.id}
                  className="group cursor-pointer hover:bg-muted/20 border-b last:border-0"
                >
                  <TableCell className="py-5">
                    <Link href={`/opportunities/${opp.id}`} className="block">
                      <div className="font-bold text-base group-hover:text-primary transition-colors leading-tight mb-1">
                        {opp.title}
                      </div>
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-1 w-24">
                      ...
                      <div className="w-full h-1 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden border border-white/5">
                        <div className="h-full bg-primary/20 w-1/2 animate-pulse" />
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button
                      ...
                      size="sm"
                      asChild
                      className="rounded-full font-bold uppercase tracking-tighter px-4 hover:bg-primary hover:text-primary-foreground transition-all"
                    >
                      <Link href={`/opportunities/${opp.id}`}>Assemble</Link>
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
